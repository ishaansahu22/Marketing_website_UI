...
export default async function InvitePage({
  ...
  // Resolve the opaque token to the actual invite code server-side.
  // The code is never placed in the URL — only shown on this page for manual entry.
  const { data: inviteCode } = await supabase.rpc("resolve_invite_token", {
    p_token: token,
  })
        ...
        {expired ? (
          <>
            ...
            <p style={{ fontSize:"14px", color:"#666" }}>
              Ask the trip organiser to share a new invite link, or enter your
              code manually under <strong>Trips → Join with code</strong>.
            </p>
          </>
        ) : (
          <>
            ...
            <p style={{ fontSize:"14px", color:"#666", marginBottom:"32px" }}>
              Someone wants you to join their trip.
            </p>
            <div style={{ background:"#f0f4ff", border:"2px dashed #6b80ff",
              borderRadius:"12px", padding:"20px", marginBottom:"24px" }}>
              <div style={{ fontSize:"12px", color:"#6b80ff", fontWeight:600,
            ...
            <a href={`daybricks://invite/${inviteCode}`}
              style={{ display:"block", background:"#1a1a1a", color:"#fff",
                textDecoration:"none", padding:"16px 24px", borderRadius:"12px",
                fontSize:"16px", fontWeight:600, marginBottom:"16px" }}>
              Open DayBricks
            </a>
            <p style={{ fontSize:"13px", color:"#999", lineHeight:1.6 }}>
              Don&apos;t have the app? Download DayBricks and enter the code above
              under <strong style={{ color:"#555" }}>Trips → Join with code</strong>.
            </p>
          </>
        )}
      </div>
